<?php
    ...
    $cols = retrieveColumns($sTable, 0, $storeConn);
    ...
    $sQuery = "SELECT TOP $limit " . implode($searchColumns, ", ") . " FROM $sTable
                $sWhere ".(($sWhere=="")?"WHERE $where AND ":" AND $where AND ")."$sIndexColumn NOT IN (
                    SELECT $sIndexColumn FROM (
                        SELECT TOP $top " . implode($searchColumns, ", ") . " FROM $sTable 
                        $sWhere ".(($sWhere=="")?"WHERE $where ":" AND $where ")."
                        $sOrder
                    ) AS [virtTable] )
                $sOrder ";
    
    $rResult = sqlsrv_query($storeConn, $sQuery, array(), array("Scrollable" => "static"));
